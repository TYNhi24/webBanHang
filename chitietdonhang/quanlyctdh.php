	...
	<div class="grid wide">
		<div class="row sm-gutter grid-content">
			...
			if (mysqli_num_rows($ketqua) > 0) {
				// Khởi tạo biến đếm $stt để đánh số thứ tự
				$stt = 1;
				// Duyệt qua từng dòng kết quả trả về từ truy vấn
				while ($row = mysqli_fetch_assoc($ketqua)) {
					// Hiển thị mỗi dòng dữ liệu trong một hàng của bảng HTML
					echo "<tr>";
					echo "<td>" . $stt . "</td>"; // Hiển thị số thứ tự
					echo "<td>" . $row['iddonhang'] . "</td>"; // Hiển thị ID đơn hàng
					echo "<td>" . $row['iduser'] . "</td>"; // Hiển thị ID khách hàng
					echo "<td>" . $row['idsp'] . "</td>"; // Hiển thị ID sản phẩm
					echo "<td>" . $row['soluong'] . "</td>"; // Hiển thị số lượng sản phẩm
					// Hiển thị nút xóa chi tiết đơn hàng
