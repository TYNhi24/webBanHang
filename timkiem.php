<?php
session_start();
// Kiểm tra dl được gửi đi bằng phương thức POST
if (isset($_POST['s'])) {
   $s = $_POST['s'];
   ...
   if (!$conn) {
    ...
    // Tạo câu truy vấn SQL để lấy thông tin từ bảng sanpham trong csdl, trong đó tensp chứa từ khóa tìm kiếm ($s). % biểu thị một phần của chuỗi.
   $sql = "SELECT * FROM sanpham WHERE tensp LIKE '%$s%'";
   $ketqua = mysqli_query($conn, $sql);
}
?>
<head>
	<title>4 GIRLS | Tìm kiếm</title>
	<meta charset="utf-8">
      ...
      <div class="header">
         <div class="grid wide">
            <div class="header-main">
               ...
               <div class="search">
						<input type="text" name="s" class="search-bar" placeholder="Nhập tên sản phẩm" value="<?php if (isset($s)) echo htmlspecialchars($s); ?>">
               ...
               <nav class="category">
                  ...
                  <ul class="category-list">
                     <li class="navbar-category-item">
								<?php
								$conn3 = mysqli_connect("localhost", "root", "", "ql_banhang");
								if (!$conn3) {
									die("Kết nối thất bại: " . mysqli_connect_error());
								}
